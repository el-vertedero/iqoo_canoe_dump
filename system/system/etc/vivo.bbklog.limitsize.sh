...
then
    /system/bin/du -s /data/vendor/audiohal/audio_dump | awk ' { if($1 > 2500000) system("rm -rf /data/vendor/audiohal/audio_dump") } '
else
...
then
    /system/bin/du -s /data/vendor/camera/dump | awk ' { if($1 > 3500000) system("rm -rf /data/vendor/camera/dump") } '
else
